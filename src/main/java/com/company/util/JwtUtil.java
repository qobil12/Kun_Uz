...
public class JwtUtil {
    ...
    public static String encode(Integer id) {
        ...
        jwtBuilder.setExpiration(new Date(System.currentTimeMillis() + ((60 * 60 * 1000)*24))); //  1 day
    ...
    public static String encode(Integer id, ProfileRole role) {
        JwtBuilder jwtBuilder = Jwts.builder();
        jwtBuilder.setIssuedAt(new Date()); // 18:58:00
        jwtBuilder.setExpiration(new Date(System.currentTimeMillis() + ((60 * 60 * 1000)*24))); // 19:58:00
        jwtBuilder.setIssuer("Mazgi production");
        jwtBuilder.signWith(SignatureAlgorithm.HS256, secretKey);
        jwtBuilder.claim("id", id);
